...
- **TRAIN-2: Raise MLX memory limit to reduce jetsam kills** — Chunk 1 training produced 64 restarts, 37 of which were jetsam kills (exit 137). At time of kill, MLX peak active was 17.73 GB against a hard limit of 14 GB — the limit was being exceeded during peak allocation. The system had physical memory to spare (free + compressor headroom); jetsam killed the process based on the declared limit, not actual pressure. Fix: raise `set_memory_limit()` in `train_ip_adapter.py` from 14 GB to 20 GB (or remove the call and let MLX manage it). Measure kill rate before/after on a smoke run. If kills persist, consider adding a `vm_stat`-based check at startup to dynamically set the limit based on current free memory.

- **TRAIN-3: Log explicit error on shard loader subprocess crash** — When the shard loader worker process crashes, the training loop waits 120 s for a `sample_q.get(timeout=120)` that never arrives, then raises `queue.Empty` which surfaces as `sample_q timeout`. The root cause is invisible in the log — the worker's exception and traceback go to the subprocess's stderr which is not captured. Fix: redirect worker stderr to a named pipe or file, log the captured output when the 120 s timeout fires. This turns a silent `sample_q timeout` into a diagnosable crash with the actual exception.
...
- **PIPE-27: Rate-limit orchestrator escalation events after 5 repeats** — During periods of stuck training (stale heartbeat, jetsam loop), the orchestrator logged identical escalation pairs every 60 s for hours (`Chunk 1: train failed twice — escalating` repeated 200+ times). This floods both the log and the dispatch queue, making it hard to spot genuinely new events. Fix: in `_dispatch_once`, after 5 dispatches of the same key, extend the cooldown from 3600 s to 7200 s. Add a counter to the `_dispatch_last` dict (or a separate `_dispatch_count` dict). Log the suppression: `"escalation for hung_train_1 suppressed (fired 6 times, next in 2h)"`.

- **PIPE-28: Fix `build_shards.py` heartbeat thread crash on InterruptedError** — Chunk 3 `build_shards` ran at Apr 25 and failed twice with `InterruptedError: [Errno 4] Interrupted system call` from `os.scandir()` in the background heartbeat thread. This was caused by a POSIX signal (likely SIGCONT from a sleep-wake event) interrupting a system call in the background thread. The main build process continued and eventually exited 0, but the orchestrator saw the heartbeat thread crash and classified the run as failed. Fix: wrap `os.scandir()` in the `_hb_loop` function with `try/except (InterruptedError, OSError)` and continue the loop. The heartbeat may miss one tick but should not crash.

- **PIPE-29: Post-precompute coverage verification before marking precompute.done** — Chunk 2 precompute left shard 200013 with ~35% of its records missing from `.npz` coverage after two partial runs, but the orchestrator didn't detect this until the promotion coverage check (`_promote_chunk`) 12+ hours later and 200+ escalation events. Fix: add an explicit coverage check at the END of `precompute_all.py` before it exits 0: count total `.npz` files in each output dir, compare to the expected record count from the shard headers, and exit 1 with a per-shard deficit report if any shard is incomplete. This converts a late-detected promotion failure into an immediate precompute failure that the orchestrator can retry automatically.

- **PIPE-30: Populate `train/eval_refs/` with reference style images** — CLIP-I (style fidelity metric) was null for every validation run across all 4 chunks because the 5 reference images referenced in `eval_prompts.txt` do not exist at `train/eval_refs/`. Without them, `validator.py` always returns `CLIP-I=None` and `verdict=WARN`, making the validation system useless for detecting style degradation between chunks. Fix: commit 5 representative reference style images to `train/eval_refs/` (watercolor_portrait.jpg, impressionist_landscape.jpg, oil_painting_style.jpg, neon_noir_style.jpg, charcoal_sketch_style.jpg — or equivalents). Images must be ≤1 MB each to keep repo size manageable; resize to 512×512 before committing.
