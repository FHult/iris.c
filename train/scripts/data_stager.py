...
class DataStager:
    ...
    def publish_precomp_from_flywheel_iter(
        self, src_precomp_base: Path, selected_shard_ids: list[str],
        training_cfg: Optional[dict] = None,
    ) -> dict:
        """
        Publish .npz generated in a flywheel iter's per-iter precomp tree to cold.

        If training_cfg is provided:
          - Uses PrecomputeCache + encoder_config_subset + version_hash to create
            a proper versioned dir on cold (e.g. cold/precomputed/qwen3/v_abc123/)
          - Copies the relevant .npz there
          - Writes manifest via write_manifest_incomplete + mark_complete
          - mark_complete also atomically updates the "current" symlink on cold
            to this new version, so the flywheel-produced data becomes the active
            default (participates in the versioned invalidation / effective_dir story)
          - Then rmtree's the per-iter precomp tree (ephemeral data is now safely
            in the durable versioned cold location)

        If no training_cfg (or error in versioning path):
          - Falls back to copying into the existing "current" target dir on cold
            (or flat).

        Always copies (never symlinks to staging). Filters to selected shards.

        Returns {"npz_published": N, "bytes_transferred": B}
        """
        from pathlib import Path as _Path
        import os as _os
        import shutil
        ...
        src_base = _Path(src_precomp_base)
        if not src_base.exists():
            return {"npz_published": 0, "bytes_transferred": 0}

        # normalize selected
        selected_ids: set[str] = set()
        for sid in selected_shard_ids:
            try:
                selected_ids.add(f"{int(sid):06d}")
        ...
        if training_cfg is not None:
            try:
                ...
                git_sha = get_git_sha(Path(__file__).parent)
                for encoder in ("qwen3", "vae", "siglip"):
                    src_enc = src_base / encoder
                    if not src_enc.exists():
                        continue
                    subset = encoder_config_subset(encoder, training_cfg)
                    pcache = PrecomputeCache(self._cold_precomp, encoder, subset, git_sha)
                    target = pcache.cache_dir()
                    target.mkdir(parents=True, exist_ok=True)

                    for npz in src_enc.glob("*.npz"):
                        try:
                            prefix = f"{int(npz.stem.split('_')[0]):06d}"
                        except Exception:
                            prefix = npz.stem.split("_")[0]
                        if prefix not in selected_ids:
                            continue
                        dst = target / npz.name
                        if dst.exists() or dst.is_symlink():
                            continue
                        nb = self._atomic_copy(npz, dst)
                        if nb > 0:
                            published += 1
                            total_bytes += nb
                    ...
                    try:
                        if target.name == pcache.version():
                            pcache.write_manifest_incomplete()
                            rec_count = sum(1 for f in target.glob("*.npz") if f.suffix == ".npz")
                            shard_set = set()
                            for f in target.glob("*.npz"):
                                try:
                                    shard_set.add(f.stem.split("_")[0])
                                except Exception:
                                    pass
                            pcache.mark_complete(record_count=rec_count, shard_count=len(shard_set))
                    except Exception:
                        pass
                did_versioned = True
            except Exception:
        ...
        if not did_versioned:
            # fallback: copy to whatever the current target dir is (flat or versioned current)
            tasks: list = []
            for encoder in ("qwen3", "vae", "siglip"):
                src_enc = src_base / encoder
                if not src_enc.exists():
                    continue
                cold_enc = self._cold_precomp / encoder
                cold_enc.mkdir(parents=True, exist_ok=True)
                cur_link = cold_enc / "current"
                if cur_link.is_symlink():
                    try:
                        ver = _os.path.basename(_os.readlink(str(cur_link)))
                        cold_target = cold_enc / ver
                    except Exception:
                        cold_target = cold_enc
                else:
                    cold_target = cold_enc
                cold_target.mkdir(parents=True, exist_ok=True)
                for npz in src_enc.glob("*.npz"):
                    try:
                        prefix = f"{int(npz.stem.split('_')[0]):06d}"
                    except Exception:
                    ...
                    if prefix not in selected_ids:
                        continue
                    dst = cold_target / npz.name
                    if dst.exists() or dst.is_symlink():
                        continue
                    tasks.append((npz, dst))
            if tasks:
                def _do(src_dst):
                    src, dst = src_dst
                    try:
                        ...
                        return 0
                with ThreadPoolExecutor(max_workers=self.max_parallel) as pool:
                    for nb in pool.map(_do, tasks):
                        if nb > 0:
                            published += 1
                            total_bytes += nb

        # rmtree the per-iter precomp after (attempted) successful publish
        # the data is now in the (versioned) cold location and reusable
        try:
            if src_base.exists():
    ...
    def _archive_dbs(self) -> None:
        """Copy hot SQLite DBs to cold metadata dir.

        Uses the SQLite backup API for a WAL-consistent snapshot.
        """
