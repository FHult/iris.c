...
class DataStager:
    ...
    def publish_precomp_from_flywheel_iter(
        ...
        """
        Copy .npz files that were generated by precompute_all.py during a flywheel
        iteration (written to a local per-iter staging/precomputed/{q,v,s} tree)
        back into the cold precomputed storage.

        This ensures the expensive encoder work (Qwen3 + VAE + SigLIP) done for
        the shards selected in this flywheel iter is not wasted: future flywheel
        iterations that re-select overlapping shards will get the caches via
        stage_iteration_shards (cold→hot), and the main chunk pipeline will also
        see them under its effective precomp dirs.

        We always *copy* (never symlink) because the source staging dir is
        ephemeral (rmtree'd at the start of the next iter with the same name).

        Only .npz whose shard prefix matches one of the selected shards for this
        iter are published (defensive; precompute may have produced exactly those).

        The target on cold follows the versioned layout if "current" symlink exists
        (same logic as staging), otherwise falls back to the flat enc dir.
        ...
        # Selected shard ID prefixes (6-digit zero-padded)
        selected_ids: set[str] = set()
        ...
        for encoder in ("qwen3", "vae", "siglip"):
            ...
            # Resolve target version dir on cold (mirrors logic in stage_iteration_shards)
            cur_link = cold_enc / "current"
            if cur_link.is_symlink():
                try:
                    ver = _os.path.basename(_os.readlink(str(cur_link)))
        ...
        # Always copy (durable on cold), never symlink to ephemeral staging.
        # Use the stager's atomic copy helper.
